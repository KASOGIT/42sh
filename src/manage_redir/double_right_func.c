/*
** double_right_func.c for  in /home/soto_a/rendu/PSU_2014_42sh/refurbish_42sh/src/manage_redir
** 
** Made by adam soto
** Login   <soto_a@epitech.net>
** 
** Started on  Fri May  8 14:50:43 2015 adam soto
** Last update Sun May 24 08:23:37 2015 adam soto
*/

#include "42sh.h"

static int	exec_double_right(t_tree **node, int fd_file)
{
  dup2(fd_file, 1);
  run_tree(&((*node)->left));
  return (EXIT_SUCCESS);
...
static int	fork_and_exec(t_tree **node, int fd_file)
{
  ...
  if ((pid = fork()) == 0)
    exec_double_right(node, fd_file);
  else
    waitpid(pid, 0, 0);
  return (EXIT_SUCCESS);
}
  ...
  if ((*node)->right && (*node)->left
      ...
      && (tab_epur_name = str_to_wordtab((*node)->right->data)) != NULL)
    {
      ...
      if ((fd_file = open(tab_epur_name[0],
			  O_WRONLY | O_CREAT | O_APPEND, g_info.mode)) == -1)
